...
def evaluate_slowfast_dynamic_batchsize(model_path : str, xpu_type: str, batchsize: int):
    
    sess_options = ort.SessionOptions()
    # sess_options.optimized_model_filepath = "/dsmtest/dsm_test/opt_slowfast_dynamic_batch.onnx"
    ...
    input_dict = {}
    input_data_1 = np.random.randn(batchsize, 3, 8, 224, 224).astype(np.float32)
    input_data_2 = np.random.randn(batchsize, 3, 32, 224, 224).astype(np.float32)

    input_dict["slow_input"] = input_data_1
    input_dict["fast_input"] = input_data_2
...
if __name__ == "__main__":
    ...
    for bs_num in batch_size_num:
        ...
        md, l2 = evaluate_slowfast_dynamic_batchsize(args.model, args.xpu, bs_num)
