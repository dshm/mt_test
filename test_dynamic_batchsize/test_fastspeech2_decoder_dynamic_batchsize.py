...
def evaluate_fastspeech2_decoder_dynamic_batchsize(model_path : str, xpu_type: str, batchsize: int):
    if xpu_type == 'musa': 
    ...
    input_dict = {}
    input_data = np.random.randn(batchsize, 100, 384).astype(np.float32)
    ...
    L2norm = np.sum(np.abs(xpu_result[0] - cpu_result[0])) / cpu_result[0].size
...
if __name__ == "__main__":
    ...
    for bs_num in batch_size_num:
        ...
        md, l2 = evaluate_fastspeech2_decoder_dynamic_batchsize(args.model, args.xpu, bs_num)
